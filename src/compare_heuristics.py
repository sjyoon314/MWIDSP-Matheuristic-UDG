"""
Utility script to compare two Heuristic models (e.g., Cross_LS vs Shift_Full).
It aggregates the Cost and computation time across different weight schemes (EG, NG, VG) 
...
def compare_and_plot():
    ...
    header = f"========== Heuristics Performance Comparison ({name1} vs {name2}) =========="
    print(f"\n{header}")
    ...
    for scheme in grouped.index:
        ...
        stats3 = f"  - Avg Time: {name1} {t_1:.4f}s vs {name2} {t_2:.4f}s"
    ...
    overall4 = f"  - Total Avg Time: {name1} {df['time_1'].mean():.4f}s vs {name2} {df['time_2'].mean():.4f}s"
    ...
    # Save text report
    txt_filename = f"comp_heuristics_{name1}_vs_{name2}.txt"
    ...
    ax.set_title(f'Cost Comparison: {name1} vs {name2} (500 Nodes)')
    ax.set_xticks(x)
