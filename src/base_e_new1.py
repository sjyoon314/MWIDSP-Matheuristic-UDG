...
def solve_new1(G, time_limit):
    ...
    model.Params.OutputFlag = 1  # 로그 표시
    ...
    # --- 수정된 부분: Exact 스탯(Obj, Bound, Gap) 추출 ---
...
def solve_all_new1(in_dir_path, instances_subset, out_dir_path):
    ...
    for file_name in sorted(os.listdir(in_dir_path)):
        ...
        print(f"\n[{file_name}] Executing NEW1 Baseline...")
        ...
        print(f" -> NEW-1 Finished | Obj: {best_obj:.1f} | Bound: {best_bound:.1f} | Gap: {gap:.2f}% | Time: {time_elapsed:.4f}s")
